<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
    <div class="relative h-full flex-1 overflow-hidden rounded-xl border border-neutral-200 dark:border-neutral-700">
        ...
        <div x-data="{ categoryModal: null }" class="flex w-full gap-4 p-4" x-init="() => {
            document.addEventListener('closeCategoryModal', () => categoryModal.hide())
        }">
            @livewire('modals.form-category')

            <button
                class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                x-init="() => {
                    const targetEl = document.getElementById('form-category-modal');
                    ...
                    categoryModal = new Modal(targetEl, options, instanceOptions)
                }" x-on:click="() => categoryModal.show()" type="button">
                New Category
            </button>
        </div>
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($categories as $category)
                        <tr x-data="{
                            isActive: @js($category->is_active)
                        }"
